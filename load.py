...
hora_db['time'] = hora_db['time'].astype(str)

dim_ubicacion = (
    df[["District", "CommunityArea", "LocationDescription", "Latitude", "Longitude"]]
    .rename(columns={
        "District": "district",
...
dim_tipo = (
    df[["IUCR", "Description"]]
    .rename(columns={"IUCR": "iucr", "Description": "description"})
    .drop_duplicates()
)
dim_tipo['iucr'] = dim_tipo['iucr'].astype(str)
dim_tipo['description'] = dim_tipo['description'].astype(str)
...
tipo_db['iucr'] = tipo_db['iucr'].astype(str)
tipo_db['description'] = tipo_db['description'].astype(str)

dim_arresto = (
    df[["Arrest"]]
    .rename(columns={"Arrest": "arrest"})
    .drop_duplicates()
)
...
df_hecho_crimenes = df[["CaseNumber", "Year", "Month", "Day", "Time", "District", "CommunityArea", "IUCR", "Description", "LocationDescription", "Arrest", "Latitude", "Longitude"]]
...
df_hecho_crimenes['Time'] = df_hecho_crimenes['Time'].astype(str)
df_hecho_crimenes = df_hecho_crimenes.merge(
    hora_db,
    how = "left",
    left_on = "Time",
...
df_hecho_crimenes['IUCR'] = df_hecho_crimenes['IUCR'].astype(str)
df_hecho_crimenes['Description'] = df_hecho_crimenes['Description'].astype(str)
df_hecho_crimenes = df_hecho_crimenes.merge(
    tipo_db,
    how = "left",
...
df_hecho_crimenes_final = df_hecho_crimenes[[
    ...
    "id_tipo",
    "id_arresto"
]].dropna(subset = ["id_fecha", "id_hora", "id_ubicacion", "id_tipo", "id_arresto"]).rename(columns={
